...
public class VarietyMethods {
	...
	{
		System.out.printf("%s(은)는 %d년 %d월 생입니다.\n",name, year, month);
	}
	 ...
	 * @param stage : int
	 */
	public void printNineNineTabl(int stage)
	{
	...
	public void printNineNineTableArray(int[] stage) {
		int element = 0;
		for (int set = 0; set < stage.length; set++) {
			element = stage[set];
			System.out.printf("%d단\n",element);
			for (int count = 1; count <= 9; count++) {
	...
	public int sumOfArray(int[] numbers) {
		...
		for (int set = 0; set < numbers.length; set++) {
			sum+=numbers[set];
		}
		return sum;
	}
	 ...
	 * @param y : int
	 */
	public int adder(int x, char op, int y) {
		int sum = 0;
		if (op == '+') {
			sum = x + y;
		} else {
